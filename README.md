...
- 🎯 Entry points to execute your program using `python -m <zihin_api>` or `$ zihin_api` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/hsdanield/zihin-api/branch/main/graph/badge.svg?token=zihin-api_token_here)](https://codecov.io/gh/hsdanield/zihin-api)
[![CI](https://github.com/hsdanield/zihin-api/actions/workflows/main.yml/badge.svg)](https://github.com/hsdanield/zihin-api/actions/workflows/main.yml)

Awesome zihin_api created by hsdanield

## Install

from source
```bash
git clone https://github.com/hsdanield/zihin-api zihin_api
cd zihin_api
make install
```
...
```bash
pip install zihin_api
```

## Executing

```bash
$ zihin_api run --port 8080
```

or

```bash
python -m zihin_api run --port 8080
```

or

```bash
$ uvicorn zihin_api:app
```
...
```bash
❯ zihin_api --help
Usage: zihin_api [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ zihin_api create-user --help
Usage: zihin_api create-user [OPTIONS] USERNAME PASSWORD
...
```bash
zihin_api create-user admin admin --superuser
```
...
Run with `zihin_api run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `zihin_api create-user`.
...
-----------------------------------------------------
zihin_api/__init__.py              4      0   100%
zihin_api/app.py                  16      1    94%
zihin_api/cli.py                  21      0   100%
zihin_api/config.py                5      0   100%
zihin_api/db.py                   10      0   100%
zihin_api/models/__init__.py       0      0   100%
zihin_api/models/content.py       47      1    98%
zihin_api/routes/__init__.py      11      0   100%
zihin_api/routes/content.py       52     25    52%
zihin_api/routes/security.py      15      1    93%
zihin_api/routes/user.py          52     26    50%
zihin_api/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from zihin_api.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in zihin_api/default.toml.

### As environment variables
```bash
export zihin_api_KEY=value
export zihin_api_KEY="@int 42"
export zihin_api_KEY="@jinja {{ this.db.uri }}"
export zihin_api_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
zihin_api_ENV=production zihin_api run
```
